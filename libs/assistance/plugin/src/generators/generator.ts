...
import { WorkspaceParameters } from '@wrkspce/assistance/feature/workspace';

export async function workspaceConfigurationGenerator(
  tree: Tree,
) {
  ...
  const configuraton: WorkspaceParameters = {
    name: '@wrkspce',
    configType: 'workspace',
    children: projectNames.filter(name => name !== 'workspace').map((projectName) => {
  ...
  const graph = {
    root: '@wrkspce',
    apps: configuraton.children.filter((child) => child.configType === 'app').map((child) => ({name: child.name, scope: child.scope, dependencies: child.dependencies})),
    libs: configuraton.children.filter((child) => child.configType === 'library').map((child) => ({name: child.name, scope: child.scope, dependencies: child.dependencies})),
  }

  const templatePath = path.join(__dirname, 'files/src');
  generateFiles(tree, templatePath, 'libs/assistance/feature/workspace/src/lib/tools/generateWorkspace', {
    config: configuraton,
...
export default workspaceConfigurationGenerator;

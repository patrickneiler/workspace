...
import { AI } from '@wrkspce/assistance/feature/core';
...
export function Purchase({
  ...
  const [history, setHistory] = useAIState<AI>();
  const [, setMessages] = useUIState<AI>();
  const { confirmPurchase } = useActions<AI>() as {
    confirmPurchase: (
      symbol: string,
      price: number,
      amount: number,
    ) => Promise<{
      purchasingUI: React.ReactNode;
